    ...
    <div className={cn('lg:sticky lg:top-24 lg:self-start', className)}>
      <div className="flex gap-3 lg:gap-4">
        ...
        <div className="relative min-w-0 flex-1">
          <div
            className="bg-muted border-border/80 relative cursor-zoom-in overflow-hidden rounded-none border"
            ...
            {media.length > 1 ? (
              <>
                ...
                <button
                  ...
                  onClick={goNext}
                  className="bg-background/95 text-foreground border-border absolute right-3 top-1/2 flex size-9 -translate-y-1/2 items-center justify-center rounded-none border shadow-sm"
                >
            ...
            <div className="hide-scrollbar mt-3 flex gap-2 overflow-x-auto pb-1 sm:hidden">
              {media.map((item, index) => (
                <button
                  ...
                  onClick={() => setActiveIndex(index)}
                  className={cn(
                    'w-16 shrink-0 overflow-hidden rounded-none border transition-all',
                    index === activeIndex
                      ? 'border-foreground border-2'
