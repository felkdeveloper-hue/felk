...
export function PromoBannerSection({ section, placement = 'home' }: PromoBannerSectionProps) {
  ...
  return (
    <section aria-label="Promotion" className="w-full py-0">
      <AsyncSection
      ...
      >
        {(item) => (
          <MotionReveal>
            <div className="bg-foreground text-background relative min-h-[22rem] w-full overflow-hidden sm:min-h-[28rem] lg:min-h-[32rem]">
              {item.imageUrl ? (
                <Image
                  src={item.imageUrl}
                  alt={item.title}
              ...
              <Container className="relative flex h-full min-h-[22rem] flex-col justify-end gap-6 py-10 text-white sm:min-h-[28rem] sm:flex-row sm:items-end sm:justify-between sm:py-14 lg:min-h-[32rem] lg:py-16">
                <div className="max-w-xl space-y-3">
                  ...
                  {item.subtitle ? <p className="max-w-md text-white/80">{item.subtitle}</p> : null}
                </div>
                {item.linkUrl ? (
                  <Button
                    asChild
