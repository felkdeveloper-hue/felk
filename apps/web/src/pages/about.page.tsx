...
/** About Us — content previously living on the Contact page. */
export function AboutPage() {
  const pageQuery = useCmsPage('about');
  const settingsQuery = usePublicSettings();
  const contactQuery = useContactInfos();
  const socialQuery = useSocialLinks();

  const about = pageQuery.data;
  ...
  const seo = extractSeo(about, settings);
  const title = seo.title ?? 'About Us';
  const description =
    seo.description ??
    'Visit Fashion Edge in Kandy — our brand story, store details, and how to reach us.';
    ...
    <>
      <Seo
        ...
        jsonLd={buildBreadcrumbJsonLd([
          ...
          { name: title, url: buildAbsoluteUrl('/about') },
        ])}
      />
