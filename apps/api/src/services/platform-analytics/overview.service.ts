...
export async function getOverview(filter: AnalyticsFilter): Promise<OverviewData> {
  ...
  const [
    ...
    countUniqueVisitorIpsFromActivity(pageCur, sessionCur, {}, eventCur, staffIds),
    countUniqueVisitorIpsFromActivity(pagePrev, sessionPrev, {}, eventPrev, staffIds),
    countUniqueVisitorIpsFromActivity(
      pageCur,
      sessionCur,
      { userId: { $ne: null } },
      eventCur,
