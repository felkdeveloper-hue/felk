    ...
    for (const product of products) {
      ...
      for (const [vIndex, variant] of productVariants.entries()) {
        ...
        sheet.addRow({
          ...
          stock,
          sku: String(v.sku ?? ''),
          ownListing: v.listSeparately ? 'TRUE' : 'FALSE',
          defaultListing: v.isDefault ? 'TRUE' : 'FALSE',
          ...
          returns: isFirst ? (p.returnsAvailable ? 'yes' : 'no') : '',
          returnPolicy: isFirst ? String(p.returnsCriteria ?? '') : '',
          warranty: isFirst ? (p.warrantyAvailable ? 'yes' : 'no') : '',
